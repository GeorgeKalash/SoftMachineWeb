...
/* Icon map from JSON keys -> Lucide icons */
...
export default function PrivacyPolicyPage() {
  const globals = siteData.globals;
  const data = siteData.privacyPolicy;

  const lastUpdated = useMemo(() => data?.lastUpdated ?? "Nov 14, 2024", [data?.lastUpdated]);
            ...
            <Card className="border-muted/40 backdrop-blur supports-[backdrop-filter]:bg-background/70 sticky top-6">
              ...
              <CardContent className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  Argus by <span className="font-medium">{globals?.company ?? "SoftMachine"}</span>.
                </div>
              </CardContent>
            </Card>
          </motion.aside>
            ...
            <Card className="border-muted/40 backdrop-blur supports-[backdrop-filter]:bg-background/70">
              <CardContent className="py-5">
                <div className="flex flex-col gap-3">
                  <p className="text-sm text-muted-foreground">{data?.intro?.body}</p>
                  <div className="flex flex-wrap items-center gap-2">
                    {(data?.intro?.badges ?? []).map((b: string) => (
            ...
            <Card className="border-muted/40">
              <CardContent className="py-6">
                <article className="space-y-8 leading-relaxed">
                  {sections.map((s, idx) => {
                    const Icon = ICONS[s.iconKey] ?? Info;
                    return (
                      <div key={s.id}>
                        <section id={s.id} className="scroll-mt-24">
                          ...
                          <p className="text-muted-foreground">
                            {/* If you want to hyperlink the contact line automatically */}
                            {s.id === "contact" ? (
                              <>
              ...
              <Card className="border-primary/30">
                <CardContent className="py-4">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">{data?.footerNote}</p>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
