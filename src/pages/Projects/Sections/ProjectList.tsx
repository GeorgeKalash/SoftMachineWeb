...
// /* -------------------------------- types -------------------------------- */
// type SectorKey = "manufacturing" | "auto_retail";
...
//   return v === "manufacturing" || v === "auto_retail";
// }
...
//             />
//           ) : (
//             <div className="h-16 w-16 sm:h-20 sm:w-20 rounded-full bg-muted grid place-items-center text-lg font-semibold text-foreground/80">
//               {getInitials(t.name)}
//             </div>
//           )}
//         </div>
...
//     auto_retail: clientsData.sectorLabels?.auto_retail ?? "Retail",
//   };

//   const toTestimonial = (c: ClientItem): TestimonialCardModel | null => {
//     const t = c.testimonial;
//     if (!t) return null;
//     const img = (c.logoKey && LOGOS[c.logoKey]) || undefined;
//     return { name: t.name, role: t.role, content: t.content, image: img };
//   };

//   const lists = useMemo(() => {
//     const manufacturing = allClients
//       .filter((i) => i.sector === "manufacturing")
//       .map(toTestimonial)
//       .filter(Boolean) as TestimonialCardModel[];

//     const auto_retail = allClients
//       .filter((i) => i.sector === "auto_retail")
//       .map(toTestimonial)
//       .filter(Boolean) as TestimonialCardModel[];

//     const all = [...manufacturing, ...auto_retail];

//     return { all, manufacturing, auto_retail };
//   }, [allClients]);
...
//     ],
//     [lists, sectorLabels.manufacturing, sectorLabels.auto_retail]
//   );

//   const counts = useMemo(() => {
//     const initial: Record<TabKey, number> = { all: 0, manufacturing: 0, auto_retail: 0 };
//     return tabs.reduce<Record<TabKey, number>>((acc, t) => {
//       const v = variant === "full" ? t.data.length : Math.min(initialCount, t.data.length);
//       acc[t.key] = v;
//       return acc;
//     }, initial);
...
//             {/* Tabs bar (works for mobile + desktop) */}
//             <div className="sticky top-0 z-20 -mx-4 mb-6 bg-background/70 px-4 py-3 backdrop-blur">
...
type ClientItem = {
  ...
  /** NEW: controls which sector tabs this item appears in.
   *  If missing/empty => shows ONLY in "all"
   */
  tabs?: SectorKey[];
};
...
export default function ProjectList({
  ...
  // Build a normalized list of cards + meta (which sector tabs to show in)
  const cardsWithMeta = useMemo(() => {
    return allClients
      .map((c) => {
        ...
        return {
          card: { name: t.name, role: t.role, content: t.content, image: img } as TestimonialCardModel,
          // tabs decide visibility inside sector tabs (missing => only All)
          tabs: Array.isArray(c.tabs) ? c.tabs.filter(isSectorKey) : [],
          id: c.id,
        };
      })
      .filter(Boolean) as Array<{ card: TestimonialCardModel; tabs: SectorKey[]; id: string }>;
  }, [allClients]);

  // Tabs should be created from sectorLabels AND only if they have data
  const tabs = useMemo(() => {
    const all = cardsWithMeta.map((x) => x.card);

    const sectorKeysFromData = Object.keys(clientsData.sectorLabels ?? {})
      .filter(isSectorKey) as SectorKey[];

    const sectorTabs = sectorKeysFromData
      .map((k) => {
        const data = cardsWithMeta.filter((x) => x.tabs.includes(k)).map((x) => x.card);
        return { key: k as TabKey, label: sectorLabels[k], data };
      })
      // don’t render empty tabs
      .filter((t) => t.data.length > 0);

    return [{ key: "all" as TabKey, label: "All", data: all }, ...sectorTabs];
  }, [cardsWithMeta, clientsData.sectorLabels, sectorLabels]);

  const counts = useMemo(() => {
    return tabs.reduce<Record<string, number>>((acc, t) => {
      acc[t.key] = variant === "full" ? t.data.length : Math.min(initialCount, t.data.length);
      return acc;
    }, {});
  }, [tabs, variant, initialCount]);
  ...
  // if current tab disappears (because data changed), fallback to "all"
  useEffect(() => {
    const exists = tabs.some((t) => t.key === tabValue);
    if (!exists) setTabValue("all");
  }, [tabs, tabValue]);
      ...
      <div className="container relative mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <Tabs value={tabValue} onValueChange={handleTabChange}>
            ...
            {tabs.map((tab) => {
              const visible = counts[tab.key] ?? 0;
              return (
                <TabsContent key={tab.key} value={tab.key} className="animate-in fade-in-50">
                  <div className="grid grid-cols-1 items-stretch gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {tab.data.slice(0, visible).map((t, i) => (
