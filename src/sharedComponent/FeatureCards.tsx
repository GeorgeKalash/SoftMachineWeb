...
import React, { useRef, useState, type ReactNode, useCallback, useEffect } from "react";
import {
  motion,
  Variants,
  useReducedMotion,
...
export type FeatureItem = {
  title: string;
  desc: string;
  preview?: PreviewKind; // kept for API stability; all render the same Lottie now
  icon?: ReactNode;
  "data-testid"?: string;
};
...
export type FeatureGridProps = {
  ...
  /** Ambient sweep (kept for API compatibility; ignored now) */
  ambient?: boolean;

  /** Override animation curves / variants (kept for API compatibility) */
  ease?: [number, number, number, number];
  containerVariants?: Variants;
  childVariants?: Variants;
...
export function FeatureGrid({
  items,
  className = "",
  ambient = true, // ignored, here only for API compatibility
  ease = DEFAULT_EASE,
  containerVariants = defaultContainer,
  childVariants = defaultFadeUp,
  inViewOnce = true,
  ...
  // Notify parent without causing layout thrash
  useEffect(() => {
...
export function FeatureCard({
  title,
  desc,
  preview = "cards", // ignored for rendering; kept for API compatibility
  icon,
  ease = DEFAULT_EASE,
  variants = defaultFadeUp,
  onHoverStart,
  ...
  return (
    <motion.div
      ...
      {...rest}
    >
      {/* Unified Lottie preview area */}
      <div className="relative h-40 w-full overflow-hidden rounded-xl bg-gradient-to-br from-slate-50 to-white ring-1 ring-slate-100 grid place-items-center">
        <UnifiedLottiePreview active={active && !rm} />
      </div>
...
function UnifiedLottiePreview({ active }: { active: boolean }) {
  const lref = useRef<LottieRefCurrentProps>(null);
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      // pause preserves last frame; use stop() to rewind if desired
      lref.current.pause();
    }
  }, [active]);
